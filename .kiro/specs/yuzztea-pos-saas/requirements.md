...
3. WHEN sebuah Session telah aktif lebih dari 12 jam tanpa aktivitas, THE Auth_System SHALL meng-invalidasi Session dan mengarahkan pengguna ke halaman /login.
...
10. IF propagasi perubahan peran atau daftar Outlet penugasan ke evaluasi akses pada Session baru tidak selesai dalam 60 detik setelah penyimpanan perubahan, THEN THE Authorization_System SHALL menolak perubahan tersebut, mengembalikan kondisi peran dan penugasan pengguna ke nilai sebelum perubahan, dan menampilkan pesan kegagalan propagasi kepada Owner.
...
3. IF salah satu field tidak memenuhi batasan validasi yang ditetapkan pada Acceptance Criteria 2, THEN THE Management_Console SHALL menolak penyimpanan, menampilkan pesan error per field yang gagal, dan tidak mengubah data Outlet sebelumnya.
...
7. IF kode_outlet yang dikirim sudah digunakan oleh Outlet lain dalam Organization, THEN THE Management_Console SHALL menolak penyimpanan, menampilkan pesan duplikasi kode_outlet, dan tidak mengubah data Outlet sebelumnya.
...
1. THE Management_Console SHALL mengizinkan hanya Owner untuk membuat, mengubah, dan menonaktifkan akun pengguna dengan peran Owner, Outlet_Manager, atau Cashier, dengan validasi username dan email unik dalam Organization serta panjang username dan email masing-masing 3-64 karakter, dan SHALL menolak operasi manajemen pengguna yang dilakukan oleh peran selain Owner kecuali sebagaimana diatur pada Acceptance Criteria 2.
...
7. WHEN harga Menu_Item diubah, THE Management_Console SHALL menyimpan riwayat harga beserta tanggal berlaku, user_id pengubah, dan harga lama, secara atomik dengan perubahan harga sehingga harga baru hanya dipersist setelah riwayat berhasil tersimpan, dengan retensi minimal 24 bulan.
...
9. IF penyimpanan riwayat harga gagal saat perubahan harga Menu_Item dieksekusi, THEN THE Management_Console SHALL membatalkan perubahan harga tersebut, mempertahankan harga sebelumnya, dan menampilkan pesan kegagalan penyimpanan riwayat kepada pengguna.
...
7. WHEN Transaction berstatus confirmed berhasil dibuat, THE POS_Module SHALL meng-trigger Inventory_System untuk pengurangan stok sesuai Requirement 6, dan IF trigger Inventory_System gagal di-fire pada saat konfirmasi, THEN THE POS_Module SHALL melakukan rollback Transaction tersebut sehingga Transaction tidak tersimpan dengan status confirmed.
8. IF Inventory_System gagal mengurangi stok setelah trigger berhasil di-fire dan Transaction berstatus confirmed, THEN THE POS_Module SHALL mengubah status Transaction menjadi pending_reconciliation tanpa melakukan rollback Transaction dan SHALL mencatat kegagalan untuk peninjauan Outlet_Manager.
...
9. IF formatting nilai mata uang pada Receipt gagal untuk salah satu field harga, subtotal, diskon, pajak, total, jumlah bayar, atau kembalian, THEN THE Receipt_Formatter SHALL menolak penghasilan Receipt tersebut dan menampilkan pesan kegagalan formatting kepada Cashier tanpa mengubah status Transaction.
...
6. THE Report_System SHALL selalu menyediakan grafik tren penjualan harian untuk rentang yang dipilih bersama dengan grafik 5 Menu_Item terlaris berdasarkan jumlah unit terjual dengan tie-breaker berdasarkan urutan abjad nama Menu_Item A-Z sebagai satu kesatuan, sehingga kedua grafik tidak ditampilkan secara terpisah.
...
5. IF jumlah percobaan menyambung ulang otomatis berturut-turut mencapai 10 kegagalan, terlepas dari jenis kegagalan (timeout, error autentikasi, server shutdown, atau penyebab lain), THEN THE Yuzztea_App SHALL berhenti mencoba otomatis, menampilkan state "disconnected_terminal", dan menampilkan tombol "Coba Lagi" untuk percobaan manual.
...
7. IF payload realtime yang diterima invalid atau tidak lengkap, THEN THE Yuzztea_App SHALL mengabaikan payload, mencoba mencatat error pada log klien, dan tidak memperbarui tampilan; dan IF pencatatan log error gagal, THEN THE Yuzztea_App SHALL tetap mengabaikan payload tanpa memperbarui tampilan, mengutamakan stabilitas sistem di atas pencatatan error.
...
2. WHILE koneksi internet terputus, THE POS_Module SHALL mengizinkan pembuatan Transaction hanya dengan status "pending_sync" hingga jumlah maksimum 500 Transaction tertunda per perangkat, dan SHALL menolak pembuatan Transaction dengan status selain "pending_sync".
...
4. IF sinkronisasi sebuah Transaction "pending_sync" gagal, THEN THE POS_Module SHALL mempertahankan Transaction tersebut sebagai "pending_sync", melakukan retry hingga 5 kali dengan jeda 30 detik antar percobaan, menahan retry counter pada nilai maksimum 5 sehingga tidak meningkat lagi setelah mencapai batas tersebut, dan menampilkan indikator gagal sinkronisasi setelah percobaan terakhir.
...
1. WHEN aksi berikut terjadi: perubahan harga Menu_Item, penghapusan atau penonaktifan Menu_Item, penyesuaian stok manual berupa penambahan atau pengurangan, refund Transaction, perubahan peran pengguna, atau perubahan penugasan Outlet, THE Yuzztea_App SHALL mencatat entri audit log dalam waktu paling lama 5 detik setelah aksi terjadi, terlepas dari apakah pengguna yang melakukan aksi memiliki izin akses atau aksi ditolak oleh Authorization_System.
...
2. THE Authorization_System SHALL menerapkan RLS sebagai mekanisme isolasi pada seluruh query yang menggunakan anon key, dengan kebijakan yang terikat pada organization_id dan outlet_id pengguna pada Session aktif, dan SHALL menjamin RLS tetap berfungsi normal untuk seluruh query meskipun service_role key secara tidak sengaja juga hadir pada bundel klien.
...
7. IF percobaan login gagal lebih dari 5 kali berturut-turut dalam 10 menit dari satu email DAN throttling untuk email tersebut telah diaktifkan secara eksplisit oleh Auth_System, THEN THE Auth_System SHALL memberlakukan jeda mulai 1 detik dan digandakan setiap kegagalan berikutnya hingga batas maksimum 5 menit sebelum percobaan berikutnya diterima.
...
9. WHILE throttling untuk satu email belum diaktifkan secara eksplisit, THE Auth_System SHALL menerima percobaan login berikutnya tanpa jeda meskipun jumlah kegagalan login pada email tersebut telah melampaui 5 dalam 10 menit.
